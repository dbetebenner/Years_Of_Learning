...
```{r}
require(datalegreyar)
values <- 4 + 2 * 1:20
fig(datafy(values, "years of learning", ignore_space=F), size=70, xlabs=c("sept", "june"), symbol=setNames(c(4,5,7,10), c("up", "up", "up", "up")))
```
...
```{r}
fig(datafy(values, "datalegreya"), size=80, xlabs=c("data", "text"))
```
